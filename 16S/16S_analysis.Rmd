...
ASV = paste("[ASV", c(1:length(rownames(meco1$tax_table))), "b] ", ASV, sep = "")
...
ASV = paste("[ASV", c(1:length(rownames(meco2$tax_table))), "b] ", ASV, sep = "")
...
ASV = paste("[ASV", c(1:length(rownames(meco3$tax_table))), "b] ", ASV, sep = "")
...
metadata <- read.table("../Metagenome metadata/sampledata_16s_meta.txt", header = T, row.names = 1, sep = "\t")
...
write.table(ASV, "../Table S3. ASVs (16S).txt", quote = F, sep = "\t")
```
...
path = "../fig 2a. Plant bar plot (Class) (16S)"
...
t1$plot_venn()
ggsave("../fig 2b. Plant venn diagramm (ASV) (16S).png", width=5, height=5, dpi=400)
```
```{r}
datatable(t1$data_details, options = list(scrollX = TRUE, scrollY = TRUE))
write.table(t1$data_details, sep = "\t", quote = FALSE, file="../Table S4. Plant venn diagramm (ASV) (16S).txt")
```
...
png("../fig 3. Heatmap Plant (16S).png", width=7, height=6, units = "in", res=400)
htmp = psq %>%
...
t3$res_diff = subset(t3$res_diff, Taxa %in% grep("human", t3$res_diff$Taxa, value = TRUE, invert = TRUE))
t3$res_diff = subset(t3$res_diff, Taxa %in% grep("mammal", t3$res_diff$Taxa, value = TRUE, invert = TRUE))
t3$res_diff = subset(t3$res_diff, Taxa %in% grep("animal", t3$res_diff$Taxa, value = TRUE, invert = TRUE))
t3$res_diff = subset(t3$res_diff, Taxa %in% grep("predatory", t3$res_diff$Taxa, value = TRUE, invert = TRUE))
...
ggsave("../fig 4. FAPROTAX (16S).png", width=12, height=10, dpi=400)
```